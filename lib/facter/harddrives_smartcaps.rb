...
if File.exists?(smartctl)
  ...
  if Facter.value(:harddrives_3ware) && Facter.value(:harddrives_3ware) != ""
    Facter.value(:harddrives_3ware).split(",").each_line { |drive|
      drvcaps = []
      cntdrv = drive.split("_")
      controller = cntdrv[1][1,cntdrv[1].length]
      drv = cntdrv[2][1,cntdrv[2].length]
      %x{#{smartctl} -d 3ware,#{drv} -A /dev/twa#{controller} 2>/dev/null|sed 1,3d}.chomp.each_line { |line|
        next if line =~ /#{ignorelines}/
        vals = line.split(" ")
        if (Float(vals[0]) rescue false)
          drvcaps <<  vals[1]
  ...
  if Facter.value(:has_aacraid)
    Dir.glob("/dev/sg*").each { |drive|
      ...
      if $?.exitstatus == 0
        ...
        drvcaps = []
        %x{#{smartctl} -d sat -A #{drive} 2>/dev/null|sed 1,3d}.chomp.each_line { |line|
          next if line =~ /#{ignorelines}/
          vals = line.split(" ")
          if (Float(vals[0]) rescue false)
            drvcaps <<  vals[1]
